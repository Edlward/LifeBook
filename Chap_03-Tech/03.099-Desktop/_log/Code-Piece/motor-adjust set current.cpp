...
namespace outputInternal
{
    ...
    /* For voltage sample, the voltage of V2.5 is precise, but V3.3 is not. V3.3 can arrive at 3.38V at motor board v0.5.
     * The voltage V2.5 can be used to calibrate the V3.3 and to calculate the exact ADC sample value.
     * D2.5 is the V2.5 ADC sample value. Kvd = 2500mV/D2.5 = Vx/Dx. Vx is the voltage to ADC channel, Dx is the Vx ADC sample value.
     * So the Dx = Vx / Kvd.
     * For current sample, the zero current output voltage Vo is stable at 1.65V for ACS71240 chip.
     * So the set current Iset to ADC value will be Ds = (Iset * Sva + Vo)/Kvd. The Iset is set current, Sva is current sensor sensitivity.
     */
    void MotorImp::vTranslateSetCurrentToAdQuanta() const volatile
    {
        int32_t si32AdSetQuanta = static_cast<int32_t>( CURRENT_SENSOR_SENSITIVITY_MICRO_VOLT_PER_A ) * si16SetCurrentInMilliA;
        si32AdSetQuanta += static_cast<int32_t>( ZERO_CURRENT_OUTPUT_VOLTAGE_IN_MICRO_VOLTAGE );
        si32AdSetQuanta /= 1000; //to milli volt
        si32AdSetQuanta *= static_cast<int32_t>( ref2V5VoltAdc->ui16GetRawAdValue() );
        si32AdSetQuanta /= static_cast<int32_t>( REFERENCE_VOLTAGE_2V5_IN_MILLI_VOLTAGE );
        si16SetCurrentInAdQuanta = static_cast<int16_t>( si32AdSetQuanta * 2 ); //from range 4095 to 8190.
    }
