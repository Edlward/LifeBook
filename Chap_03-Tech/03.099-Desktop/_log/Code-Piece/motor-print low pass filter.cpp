...
namespace outputInternal
{
    ...
    volatile int8_t                           MotorImp::cui8FilterShift     = 2;
    volatile int16_t                          MotorImp::si16Adc1ZeroDrift     = 0; //better to initialize to 0, avoid not calibration.
    volatile int16_t                          MotorImp::si16Adc1ZeroDriftFilter = 0; //better to initialize to 0, avoid not calibration.
    ...
    volatile int16_t                          MotorImp::si16Adc2ZeroDriftFilter = 0; //better to initialize to 0, avoid not calibration.
    volatile int16_t                          MotorImp::si16Adc2ZeroDriftAvg  = 2048;
    ...
    {
        return (si16Adc1ZeroDriftFilter>>cui8FilterShift);
    }

    int16_t MotorImp::si16GetMotorAdcDrift2Avg() const volatile
    {
        return (si16Adc2ZeroDriftFilter>>cui8FilterShift);
    }
    ...
    void MotorImp::vAdcEndOfConversionInterruptHandler ( void* param )
    {
        ...
        else
        {
            ...
            //filter
            si16Adc1ZeroDriftAvg = si16GetZeroDriftFilterValue( si16Adc1ZeroDrift, si16Adc1ZeroDriftFilter);
            si16Adc2ZeroDriftAvg = si16GetZeroDriftFilterValue( si16Adc2ZeroDrift, si16Adc2ZeroDriftFilter);
        }
    ...
    int16_t MotorImp::si16GetZeroDriftFilterValue( const int16_t si16DriftValue, volatile int16_t &si16FilterValue )
    {
    ...
    uint16_t MotorImp::ui16GetCalibrationSampleValue( const uint16_t ui16SampleValue )
    {
        ...
        {
            if (( csi32Ref2v5RawValue > static_cast<int32_t>( ADC_QUANTA_OF_2V5_WITHIN_3V3 + 10 ))
             || ( csi32Ref2v5RawValue < static_cast<int32_t>( ADC_QUANTA_OF_2V5_WITHIN_3V3 - 10 ))) //out of range to adjust.
            {
